"""Main experiment script: Baseline vs EntNorm on JacRED dev subset."""
...
from extraction import run_baseline, run_entity_normalized
...
def run_condition(name, docs, few_shot, client, schema_info,
    ...
    Args:
        extraction_fn: "baseline" or "entity_normalized"
        constraint_table: required when extraction_fn is "entity_normalized"
    """
    print(f"\n--- {name} ---")
    per_doc_results = []
    ...
    for i, doc in enumerate(docs):
        ...
        if extraction_fn == "entity_normalized":
            entities, triples, stats = run_entity_normalized(
...
def main():
    ...
    entnorm_results = run_condition(
        "Condition 2: EntNorm (Mention-Span Constrained Entity Normalization)",
        dev_docs, few_shot, client, schema_info,
        extraction_fn="entity_normalized",
        constraint_table=constraint_table,
    )

    # Comparison table
    b = baseline_results["aggregate"]
    e = entnorm_results["aggregate"]
    print("\n=== Comparison ===")
    ...
    print(f"{'EntNorm':>12} {e['precision']:>10.2f} {e['recall']:>8.2f} {e['f1']:>6.2f} {e['tp']:>5} {e['fp']:>5} {e['fn']:>5}")
    ...
    output = {
        ...
        "conditions": {
            ...
            "entity_normalization": entnorm_results,
        },
    }
